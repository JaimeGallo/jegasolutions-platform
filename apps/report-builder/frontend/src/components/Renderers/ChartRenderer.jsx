...
const ChartRenderer = ({ component, excelData: propsExcelData }) => {
  ...
  if (component.dataSource?.sourceType === "excel" && excelData?.data) {
    if (!excelData.headers || excelData.headers.length === 0) {
      ...
      return <div className="p-4 text-gray-500">No hay datos de Excel disponibles o válidos.</div>;
    }
    ...
    if (xAxisField && yAxisField) {
      ...
      if (seriesIndex >= 0 && seriesField && component.chartType !== "pie") {
        ...
        chartData = excelData.data.map((row, index) => {
          const item = {};

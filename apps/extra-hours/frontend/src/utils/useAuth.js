...
export const useAuth = () => {
  ...
  // NUEVO: Obtener email del usuario autenticado desde el token
  const getUserEmail = () => {
    if (auth?.token) {
      try {
        const decodedToken = jwtDecode(auth.token);
        return decodedToken.unique_name || decodedToken.email || null;
      } catch (error) {
        console.error(
          "Error al decodificar el token para obtener el email:",
          error
        );
      }
    }
  ...
  // NUEVO: Obtener nombre del usuario autenticado desde el token
  const getUserName = () => {
    if (auth?.token) {
      try {
        const decodedToken = jwtDecode(auth.token);
        return decodedToken.name || decodedToken.unique_name || null;
      } catch (error) {
        console.error(
          "Error al decodificar el token para obtener el nombre:",
          error
        );
      }
    }
